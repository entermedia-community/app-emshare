...
updatebasket = function(e) {
	...
	$("#" + targetDiv).load(
			...
			function() {
				$("#basket-paint").load(
						apphome + "/components/basket/menuitem.html");
				if (action == 'remove') {
					$(".selectionbox:checked").closest("tr").hide("slow");
					$(".selectionbox:checked").closest(".emthumbbox").hide(
							"slow");
					jQuery(".selectionbox:checked").closest(
							".grid-gallery-checkbox").hide("slow");
...
onloadselectors = function() {
	...
	lQuery("a.updatebasketmediaviewer").livequery('click',
	...
	lQuery(".desktop-send").livequery(
			...
			function() {
				...
				jQuery.ajax({
					url : apphome
							+ "/components/desktop/sendcommand.html?command="
							+ json
				});
							...
							theinput
									.autocomplete({
										...
										select : function(event, ui) {
											...
											// submitted
											$("#hiddenaddmygroupusers").val(
													ui.item.value);
											var targetdiv = $(
													"#hiddenaddmygroupusers")
													.attr("targetdiv");
											var targeturl = $(
													"#hiddenaddmygroupusers")
													.attr("postpath");
					...
					function() {
						...
						if (theinput && theinput.autocomplete) {
							...
							theinput
									.autocomplete({
										source : apphome
												+ '/components/autocomplete/usersuggestions.txt',
										select : function(event, ui) {
											// set input that's just for display
											// purposes
							...
							theinput
									.autocomplete({
										...
										select : function(event, ui) {
											...
											// actually used when the form is
											// submitted
											$("#hiddenaddmygroups").val(
													ui.item.value);
											var targetdiv = $(
													"#hiddenaddmygroups").attr(
													"targetdiv");
											var targeturl = $(
													"#hiddenaddmygroups").attr(
													"postpath");
				...
				.livequery(function() {
					$(this).draggable({
						helper : function() {
							...
							if (n > 1) {
								cloned.append('<div class="dragcount emnotify">+'
												+ n
												+ '</div>');
									...
									.draggable(
											{
												...
												helper : function() {
													...
													if (n > 1) {
														cloned
																.append('<div class="dragcount emnotify">+'
																		+ n
																		+ '</div>');
						...
						.droppable(
								{
									drop : function(event, ui) {
										...
										$.get(searchhome + "/savecolumns.html",
												options, function(data) {
													var cell = findclosest(
															resultsdiv,
															resultsdiv);
													resultsdiv
															.replaceWith(data);
	...
	if (jQuery.fn.droppable) {
		...
		lQuery(".assetdropcategory .categorydroparea")
				.livequery(
						function() {
							$(this)
									.droppable(
											{
												drop : function(event, ui) {
													...
													var categoryid = node
															.parent().data(
																	"nodeid");
													var targetcategoryid = ui.draggable
															.data("nodeid")
													...
													if (targetcategoryid) {
														...
														params['oemaxlevel'] = "1";
														params['tree-name'] = tree
																.data("treename");

														jQuery
																.get(
																		apphome
																				+ "/components/emtree/movecategory.html",
																		params,
																		function(
																				data) {
																			tree
														...
														jQuery
																.get(
																		apphome
																				+ "/components/categorize/addassetcategory.html",
																		{
																			assetid : assetid,
	...
	$(this).find(".autosubmited").change(function() {
		...
		function() {
			...
			$(document).ajaxError(
					function(e, jqxhr, settings, exception) {
						...
						if (exception == 'abort' || exception == 'timeout') { // options
																				// are
																				// "timeout",
																				// "error",
																				// "abort",
									...
									.droppable(
											{
												drop : function(event, ui) {
													...
													jQuery
															.get(
																	apphome
																			+ "/components/libraries/addasset.html",
																	{
																		assetid : assetid,
																		libraryid : libraryid,
																		hitssessionid : hitssessionid
				...
				var handler = function(event) {
					if (event.dataTransfer
						...
						event.dataTransfer.setData(
								'application/x-moz-file-promise-url', urls);
						event.dataTransfer.setData(
						...
						var download = "application/force-download:" + filename
								+ ":" + urls;
				...
				$(this).droppable(
						{
							drop : function(event, ui) {
								...
								var hitssessionid = $("#resultsdiv").data(
										"hitssessionid");
								var collectionName = anode.find(
										"a.librarylabel")
										.data("collectionname");
								...
								if (!hitssessionid) {
									hitssessionid = $("#main-results-table")
											.data("hitssessionid");
								}
											...
											drop : function(event, ui) {
												...
												if (typeof categortyid != "undefined") {
													var categoryName = ui.draggable
															.data("categoryname");
													dropsaveurl = apphome
															+ "/components/opencollections/dropcategory.html";
