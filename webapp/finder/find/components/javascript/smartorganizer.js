...
$(document).ready(function () {
  ...
  function initializeSmartOrganizer() {
	...
	*/
      ...
      function handleSelect(selectedGroup = null) {
        ...
        if (selectedGroup) {
          if (selectedGroup.cssClass === "folderGroup") {
            ...
            $("#folderDesc").val(
              selectedLabel.getUserData()?.description || ""
            );
      ...
      $(document).on("click", "button.icn-icn", function (e) {
        e.preventDefault();
        e.stopImmediatePropagation();
        showLoader();
  ...
  lQuery("#iconsList").livequery(function () {
    if ($(this).children().length < 2050) {
      ...
      for (var i = 0; i < bsIcons.length; i++) {
        icHtm += `<div><button class="btn icn-icn"><img src="${apphome}/theme/icons/bootstrap/${bsIcons[i]}.svg" loading="lazy"/></button><span>${bsIcons[i]}</span></div>`;
      }
      $(this).html(icHtm);
    }
  });
